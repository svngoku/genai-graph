"""Unit tests for MarkdownTreeFactory (source resolution + document parsing)."""
...
@pytest.mark.unit
class TestMarkdownTreeFactorySchema:
    def test_build_schema_has_document_and_section(self) -> None:
        factory = MarkdownTreeFactory(sources=["/tmp/does-not-matter"])
...
@pytest.mark.unit
class TestMarkdownTreeFactorySources:
    ...
    def test_single_file_source(self, tmp_path: Path) -> None:
        ...
        factory = MarkdownTreeFactory(sources=[str(md_file)])
        keys = factory.get_keys()
    ...
    def test_zip_source_is_extracted_and_scanned(self, tmp_path: Path) -> None:
        ...
        factory = MarkdownTreeFactory(sources=[str(zip_path)], cache_dir=str(tmp_path / "cache"))
        keys = factory.get_keys()
    ...
    def test_mixed_sources_deduplicated(self, tmp_path: Path) -> None:
        ...
        factory = MarkdownTreeFactory(sources=[str(tmp_path), str(md_file)])
        keys = factory.get_keys()
...
@pytest.mark.unit
class TestMarkdownTreeFactoryParsing:
